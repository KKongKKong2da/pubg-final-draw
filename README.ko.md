...
| **응원 지정** | 진행 중 참가자를 누르면 같은 이름의 티켓 전원이 강조되고 나머지는 흐려집니다. 다시 누르면 해제되고, 다른 이름을 누르면 그쪽으로 옮겨갑니다. 보기를 돕는 기능이며 결과에는 영향을 주지 않습니다. |
...
복사 결과에는 앱 버전, 알고리즘 버전, 시드, 명단 해시, 참가자 수와 티켓 수가 함께 기록됩니다. 재현을 좌우하는 것은 추첨 알고리즘 버전이며, 앱 버전은 기록용입니다. 앱 버전이 올라가도 알고리즘 버전이 같으면 같은 시드는 같은 결과를 냅니다.

> **v1.0.0 이전 프로토타입과 프리릴리스는 동일한 시드라도 결과가 다를 수 있습니다. v1.0.0부터 `los-shuffle-v2`를 기준으로 결과 호환성을 유지합니다. 예를 들어 v1.0.0과 v1.1.0은 알고리즘이 같으므로 동일한 시드에서 동일한 결과를 냅니다.**
...
### v1.1.0 - 응원 지정 추가 (2026-07-27)

- **응원 지정 기능**: 진행 중 참가자(점 또는 이름표)를 누르면 같은 이름의 티켓 전원이 강조되고 나머지는 흐려집니다. 다시 누르면 해제, 다른 이름을 누르면 이동, 빈 곳을 누르면 전체 해제됩니다. 같은 이름을 여러 장 넣었을 때 시선이 분산된다는 피드백을 반영했습니다.
- 보기를 돕는 기능이며 결과·시드 재현에는 영향을 주지 않습니다.
