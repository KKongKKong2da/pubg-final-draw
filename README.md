...
| **분산 이동과 관전 리듬** | 참가자는 원 중심 한 점이 아니라 안전지대 경계 안쪽의 서로 다른 위치로 이동합니다. 이동, 접근, 격전, 이탈, 휴식이 반복됩니다. |
...
| **고정 페이싱과 속도 조절** | 1배속 약 60초, 기본 2배속 약 30초입니다. 0.5배부터 3배까지 조절할 수 있고, 진행 중 속도를 바꾸어도 시간이 튀지 않습니다. |
...
| **단일 파일·모바일 지원** | 지도 이미지와 코드가 모두 `index.html` 안에 들어 있습니다. 모바일에서는 킬로그를 지도 아래 별도 영역에 두고 최근 3건만 표시하며, 기록이 없을 때는 영역을 숨깁니다. |
...
- 입력칸을 비우면 시작할 때마다 새로운 랜덤 시드를 만듭니다.
...
단순히 점이 사라지는 대신 가까운 참가자와 대치하고, 비살상 교전을 벌이고, 교전 후 흩어져 쉬었다가 다른 상대와 다시 만나는 흐름을 추가했습니다. 자기장 밖 참가자는 안전지대로 이동하고, 늦은 참가자는 경계 근처까지 달려가다가 자기장에 탈락하도록 연출했습니다.
...
전체 순위 보존, 순위 복사, 시드 입력과 재현, 참가자·티켓 수 표시, 입력 오류 안내, 모바일 레이아웃을 추가했습니다. 모바일 킬로그는 전장을 가리지 않도록 지도 아래로 분리하고 최근 3건만 보여주도록 정리했습니다. 마지막으로 결과·배치·이동 난수를 분리하고, 고정 시간 스텝과 Web Crypto 기반 랜덤 시드를 적용해 v1.0.0 구조를 확정했습니다.
...
3. 필요하면 속도와 시드를 설정합니다. 시드를 비우면 랜덤으로 시작합니다.
...
| **진행 시간** | 기본 2배속에서 약 30초에 종료되고, 진행 중 속도를 변경해도 시간이 앞뒤로 튀지 않는지 확인 |
| **관전 연출** | 초기 관찰과 흰 원 예고, 6단계 수축, 분산 이동, 교전·자기장 탈락, 후반 확대가 순서대로 작동하는지 확인 |
| **소수 인원** | 3명 경기에서 이동·격전·이탈·휴식이 반복되고, 자기장 밖 참가자가 진입하며, 모든 생존자가 화면에 계속 보이는지 확인 |
| **화면 대응** | 데스크톱은 전장이 가용 영역을 채우는지 확인. 모바일은 킬로그가 전장을 덮지 않고 지도 아래에 나타나며, 최근 3건만 한 줄로 표시되고 기록이 없을 때 영역이 숨겨지는지 확인 |
...
- **PUBG 관전 연출 완성**: 에란겔 주요 지역 낙하, 6단계 자기장, 분산 이동, 교전·자기장 탈락, 후반 카메라
- **실사용 기능 완성**: 전체 순위, 복사, 시드 재현, 입력 검증, 속도 조절
- **배포 형태 확정**: 데스크톱·모바일 대응, 모바일 전장 아래 최근 3건 킬로그, 단일 `index.html` 실행과 GitHub Pages 지원
