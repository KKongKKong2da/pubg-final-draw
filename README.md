...
| **단일 파일·모바일 지원** | 지도 이미지와 코드가 모두 `index.html` 안에 들어 있습니다. 데스크톱과 모바일에서 별도 설치 없이 실행됩니다. |
...
전체 순위 보존, 순위 복사, 시드 입력과 재현, 참가자·티켓 수 표시, 입력 오류 안내, 모바일 레이아웃을 추가했습니다. 마지막으로 결과·배치·이동 난수를 분리하고, 고정 시간 스텝과 Web Crypto 기반 랜덤 시드를 적용해 v1.0.0 구조를 확정했습니다.
...
| **화면 대응** | 데스크톱은 전장이 가용 영역을 채우고, 모바일은 맵 → 탈락 기록 → 설정 순서로 배치되는지 확인 |
...
- **배포 형태 확정**: 데스크톱·모바일 대응과 단일 `index.html` 실행, GitHub Pages 지원
